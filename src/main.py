...
class Window(QTG.QWidget, ui_simion_optimizer.Ui_Form):
    ...
    def __init__(self, parent = None):
        ...
        self.path_simion = os.path.join("..", "ftp")
...
class Optimizer(QTC.QThread):
    ...
    def run(self):
        ...
        if use_voltage_only == False:
            ...
            self.nr_of_geom_particles = 0
            self.nr_of_geom_iterations = 1

        init_optimizer_db(number_of_bad_news,
                          bad_news_difference,
                          self.number_of_particles,
                          self.max_number_of_voltage_iterations,
                          anz_v_pro_pa,fix_voltages,
                          fix_electrodes,
                          iob_filename,
                          PA_filenames,
                          self.nr_of_geom_particles)
        ...
        if use_voltage_only == True:
            ...
            apsa(self.voltage_optimization,
                 self.G,
                 xmin=self.gmin,
                 xmax=self.gmax,
                 number_of_particles=self.nr_of_geom_particles)
